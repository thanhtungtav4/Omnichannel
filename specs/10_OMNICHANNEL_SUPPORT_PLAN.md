...
- 2026-07-06 | Internal notes in thread (HubSpot Comment) | done | Reply vs Comment in the composer (HubSpot Conversations pattern). Comment = internal note between agents, NEVER sent to the customer, interleaved in the thread by time as a yellow sticky. internal_notes table + model already existed; added InternalNote.author relation, ConversationActionController.comment (workspace guard only — no reply-permission, it's collaboration), route, and merged notes into the thread payload (concat + sortBy created_at ms, flagged internal, id prefixed note-). MessageBubble renders internal notes centred/warn-toned. Composer toggle Trả lời/Ghi chú nội bộ: comment mode hides image attach, changes label/placeholder/send button, no image. 1 test (any agent can comment on another's conversation, 0 messages queued). Assessment: our inbox already matches most of HubSpot's flow (shared multi-channel inbox, CRM context panel, assignment/transfer, presence, filters/search) — internal notes was the main missing collaboration piece. Chatbot/AI deferred (separate module, YAGNI for dental now). | tests: 75 php + 4 sidecar
- 2026-07-06 | HubSpot-style inbox UI | done | Learned from HubSpot Conversations layout (couldn't read the screenshot — model has no image input — used known HubSpot 3-pane pattern). (1) Right rail rebuilt as an "About contact" panel like HubSpot's: big avatar + name + source, contact info rows, channel-identity chips, open Leads (deals), pinned CSKH notes, other conversations (channel history, click to switch), then transfer + full-record button — all in a ScrollArea. Backend adds leads/pinnedNotes/otherConversations + lastInboundAt to the activeConversation.contact payload (relations eager-loaded). (2) Inline assign dropdown on the thread header (HubSpot puts the owner picker there) — submitTransfer now takes an optional userId so the picker assigns immediately. (3) Unread state in the queue was already bold + dot + tinted row. PanelSection/PanelRow helpers added. | tests: 75 php + 4 sidecar
- 2026-07-06 | Unify notes (one store) | done | "Ghi chú nội bộ" (thread, internal_notes) and "Ghi chú CSKH" (contact, contact_notes) were two separate systems — confusing. Merged into ONE store: contact_notes. Added nullable conversation_id (additive migration) so a note typed from a conversation shows inline in that thread AND on the contact record. comment() now writes ContactNote (contact_id + conversation_id + pinned), requires the conversation to have a contact. Thread payload loads contact_notes for this conversation; the right-rail "pinned notes" already reads contact.notes so it now includes conversation notes automatically. Deleted the InternalNote model (internal_notes table left in place, empty, harmless). UI relabeled "Ghi chú nội bộ" → "Ghi chú khách (lưu vào hồ sơ)". Test updated to assert contact_notes. | tests: 75 php + 4 sidecar
- 2026-07-06 | Notes to right-rail only + customer tags | done | Per user preference, notes no longer interleave in the inbox thread (removed the note concat + MessageBubble internal branch + payload fields) — notes live on the right rail (pinned) and the contact page. NEW: customer tags an agent sets freely (VIP, Nợ tiền, ...). ponytail: jsonb `tags` array on contacts (no tags table/pivot — no management screen needed), Contact casts array, ContactController.updateTags (trim + case-insensitive dedup + max 20, workspace guard), PUT route. Reusable TagEditor component (chips + add box + remove, PUTs the whole list) on the inbox right rail AND contact-show header. tags added to activeConversation.contact + contactShow payloads. 2 tests (dedup/trim, cross-workspace forbidden). | tests: 77 php + 4 sidecar
- (next: Low-pri UI: routing-queue management screen, Reverb realtime, multi-nick picker UI, media S3 mirror, group backfill. Telegram/Messenger profile+avatar deferred. Chatbot/auto-reply = future module. Tag filter/search in queue if tags get heavy use.)
