...
> - **Project-original source:** https://github.com/garrytan/gstack/blob/main/design-html/SKILL.md
...
## CRM-specific design patterns (Laravel Inertia + shadcn/ui)

### Detection: this repo uses Inertia React, not vanilla HTML

When this skill says "generate HTML", the actual output is a **React component**
served via Inertia. Translation rules:

| gstack /design-html says | CRM reality |
|---|---|
| "Generate index.html" | Create `resources/js/Pages/{Module}/{Page}.tsx` + corresponding controller |
| "Use Pretext computed layout" | Use Flex/Grid + shadcn `<Resizable>` / `<ScrollArea>` for reflow |
| "30KB zero deps" | Use only shadcn/ui components + Tailwind tokens — no new deps |
| "CSS-in-JS" | Tailwind v4 + semantic tokens from `.ui-craft/tokens.css` |
| "Open in browser" | `npm run dev` → `http://localhost:8000/{path}` |

### Required shadcn components for admin UI

Reference: `specs/07_ADMIN_UI.md` § Component Inventory. Always use these (already
installed) before custom markup:

- Layout: `Card`, `Tabs`, `Sheet`, `Drawer`, `Dialog`, `Sidebar`, `Separator`, `ScrollArea`
- Data: `Table`, `Pagination`, `Badge`, `Avatar`, `Empty`, `Skeleton`, `Spinner`
- Forms: `Field`, `FieldGroup`, `InputGroup`, `Select`, `ToggleGroup`, `Tooltip`
- Feedback: `Alert`, `Sonner` (toast), `Chart` (sparklines only)
- Pattern: `cn()`, `gap-*`, `size-*`, `truncate`, `font-variant-numeric: tabular-nums`

### Hard rules (mirror .ui-craft/brief.md)

1. **Status over decoration** — color is signal only. ≤5 accent placements per viewport.
2. **Use 5 status token sets** for SLA / delivery / channel / assignment / job state.
3. **Numbers are tabular** — `font-variant-numeric: tabular-nums` + right-aligned.
4. **Every operational state visible without logs** — failed jobs, SLA breach, unassigned.
5. **Dark mode is intentional** — never raw palette inversion.

### Common admin UI patterns (already shipped)

- **Stat-strip** — compact alert row at top of queue panel: `⊗3  👤✕2` (alert count first)
- **Command composition** — work queue/table dominates (60%+ viewport), metrics are a
  compact strip, charts are sparklines only
- **Inbox queue** — left rail with filter tabs (Tất cả / Của tôi / Chờ / Ưu tiên),
  conversation list, message thread on the right

### Output format for design work

When asked to "design" or "mockup" in this repo, deliver:

1. **Inertia page** at `resources/js/Pages/{Module}/{Page}.tsx`
2. **Route + Controller** wiring (POST → Inertia::render)
3. **Mockup screenshot** via Playwright if user wants visual preview
4. **Component composition rationale** — which shadcn components + why

Never deliver standalone HTML — it won't integrate with the Laravel/Inertia pipeline.
