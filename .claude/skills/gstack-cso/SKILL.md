...
> **⚠️ Vendored mode — global binaries NOT available.**
> This skill was vendored from gstack v1.58.5.0 into `.agents/skills/` + `.claude/skills/`.
> The original preamble references `~/.claude/skills/gstack/bin/*` binaries which DO NOT
> exist in vendored mode. Those commands will fail silently (`|| true` guards them).
> AI agents loading this skill should:
> 1. **Skip the bash preamble entirely** — it's metadata only.
> 2. **Use direct tool calls** (Read/Edit/Write/Bash/Grep/Glob) instead of the gstack binaries.
> 3. **Apply the methodology described in the body** — the phases, the heuristics, the
>    output format. None of that requires the gstack runtime.
>
> **CRM context (Laravel 12 + Inertia React + shadcn/ui Base UI):**
> - Stack: Laravel 12, Inertia React, TypeScript, PostgreSQL, Redis, multi-tenant `*.qrf.vn`
> - **Read first before this skill:** `@AGENTS.md` (project rules), `@ETHOS.md` (builder principles)
> - For UI work: also `@.ui-craft/brief.md` + `@.ui-craft/tokens.css` + `@specs/07_ADMIN_UI.md`
> - For security work: also `@specs/08_RBAC.md` (RBAC), `@specs/05_CONNECTORS_*` (webhook payloads)
> - For QA: target staging at `https://<workspace>.qrf.vn/admin` or local `http://localhost:8000/admin`
> - **Project-original source:** https://github.com/garrytan/gstack/blob/main/cso/SKILL.md
> - **Refresh from upstream:** `bash scripts/update-gstack-skills.sh` (after pulling upstream)
...
# 📌 CRM-Specific Appendix (auto-appended for vendored mode)

This appendix augments the upstream skill with Laravel/CRM-specific patterns.
The methodology above is still authoritative — these notes tell you HOW to apply
it inside this repo.

## Project environment

- **Local dev:** `php artisan serve` + `npm run dev` (Vite) — admin at `http://localhost:8000/admin`
- **Staging:** `https://{workspace-slug}.qrf.vn/admin` (multi-tenant wildcard)
- **Stack detection helpers:**
  - Backend: `php artisan about` (Laravel 12)
  - Frontend: `cat package.json | jq '.dependencies'`
  - DB: `php artisan db:show --counts` (PostgreSQL)
  - Queue: `php artisan queue:monitor`
- **Test commands:**
  - Laravel feature/integration: `php artisan test --filter={TestClass}`
  - PHP unit: `vendor/bin/phpunit`
  - JS/TS: `npm test` (Vitest) or `npm run test:e2e` (Playwright)
  - Browser QA: `npx playwright test --headed`

## Cross-cutting concerns (read @AGENTS.md for full rules)

1. **Modular architecture** — every feature lives inside a module boundary
   (`app/Modules/{Module}/`). Cross-module access goes through public contracts,
   events, or services. Do NOT add code to `app/Http/Controllers/` directly when
   building module features.
2. **Tenant isolation** — every workspace-scoped query MUST filter by `workspace_id`.
   Use the `BelongsToWorkspace` scope. Audit any new query.
3. **Webhook idempotency** — every inbound webhook route MUST use the `IdempotentWebhook`
   middleware keyed on provider event ID. Pattern in `app/Modules/Channels/Http/Middleware/`.
4. **shadcn/ui rules** — no raw palette utilities for status. Use the 5 status token
   sets from `.ui-craft/tokens.css` (`--status-ok/warn/danger/info/idle`).
5. **State machines canonical in domain layer** — UI may disable actions but never be
   the only enforcement. Check `@specs/02_MODULE_STRUCTURE.md`.
6. **Every region needs 4 states:** loading (Skeleton), empty (Empty), error (Alert),
...
| What | Path |
|------|------|
| Module skeleton | `app/Modules/{Name}/` |
| Migrations | `database/migrations/` (laravel default) + per-module `app/Modules/{Name}/Database/Migrations/` |
| Routes | `routes/web.php`, `routes/api.php`, `routes/channels.php` |
| Policies | `app/Policies/` + per-module `app/Modules/{Name}/Policies/` |
| Inertia pages | `resources/js/Pages/{Module}/` |
| Inertia controllers | `app/Modules/{Name}/Http/Controllers/` |
| shadcn components | `resources/js/components/ui/` (from `npx shadcn@latest add`) |
| Custom components | `resources/js/components/{module}/` |
| Feature tests | `tests/Feature/{Module}/` |
| Unit tests | `tests/Unit/{Module}/` |
| Factories | `database/factories/` + per-module `app/Modules/{Name}/Database/Factories/` |



## CRM-specific security patterns (Laravel + multi-tenant)

### High-risk areas unique to this CRM

1. **Multi-tenant data isolation (CRITICAL)**
   - Every Eloquent query MUST filter by `workspace_id` (use `BelongsToWorkspace` scope)
   - Audit any query that doesn't go through the scope. Common leak vectors:
     - `Contact::find($id)` without workspace check → add `Gate::authorize` or scope
     - `Cache::get("user:$id")` without tenant prefix → leak across workspaces
     - Queue jobs that re-fetch by ID without workspace context
   - Test: assert user in workspace A cannot read/write workspace B data

2. **Webhook ingestion (CRITICAL — external attack surface)**
   - All inbound webhooks (`/webhooks/zalo`, `/webhooks/telegram`, `/webhooks/shopee`...)
     MUST validate signature (HMAC-SHA256 with shared secret)
   - All MUST be idempotent on `provider_event_id` (deduplication table or Redis SETNX)
   - All MUST validate payload schema at the boundary (FormRequest or custom validator)
   - Rate limit per provider IP range + per workspace
   - Test: replay same payload 10× → 1 DB row, 10× 200 OK
   - Test: invalid signature → 401 + log, NO DB write
   - Test: oversized payload → 413, NO DB write

3. **RBAC + workspace authorization (HIGH)**
   - All routes have `auth` + `workspace` middleware
   - Operator cannot access `/admin/settings/*` (admin only)
   - Workspace switcher resets cached scopes when switching
   - API tokens (Sanctum/Passport) scoped per workspace + per role
   - Test: cross-workspace API call → 403

4. **Database (PostgreSQL specific)**
   - Use parameter binding (Laravel Eloquent does this by default)
   - Audit raw `DB::statement()` / `DB::select()` calls
   - Verify JSONB columns aren't accepting unvalidated user input
   - Verify `workspace_id` is indexed on every tenant-scoped table

5. **Queue / async jobs**
   - Webhook payload processing goes through jobs (NOT inline in controller)
   - Jobs must be idempotent (use `ShouldBeUnique` or dedup table)
   - Failed jobs must trigger alert (Sentry / custom)
   - Test: kill a job mid-processing → replay → no duplicate side effects

6. **Secrets management (HIGH)**
   - `.env` is local-only — never committed (verify `.gitignore` covers it)
   - Production secrets in `.env.production` (deploy-managed, not in repo)
   - GitHub Actions SSH key for deploy — verify it's restricted to deploy commands only
   - Audit `git log -p` for accidentally committed secrets
   - Laravel `config()` returns env at boot — verify no `.env.example` includes real values

7. **Infrastructure / deploy (per `deploy/` directory)**
   - VPS at `180.93.115.225` (demo only — prod is `*.qrf.vn`)
   - Nginx wildcard + Let's Encrypt auto-renew
   - GitHub Actions SSH deploy — verify only main branch can deploy
   - PostgreSQL + Redis — verify not exposed to public internet
   - SSH key in CI — verify scoped, rotated, no `passphrase: ""` in plaintext

### Standard CSO audit additions for Laravel

- Run `composer audit` for known CVEs in dependencies
- Run `npm audit` for frontend deps
- Run `php artisan config:show` to verify no secrets in production config
- Verify `APP_DEBUG=false` in `.env.production`
- Verify `APP_KEY` is 32 chars (Laravel default), not a placeholder
- Verify `SESSION_SECURE_COOKIE=true` in production (HTTPS only)
- Verify CSRF middleware enabled on all state-changing routes
- Verify CORS config restricts to known origins (admin.qrf.vn + workspace subdomains)

### Severity classification

Use the upstream gstack severity (CRITICAL/HIGH/MEDIUM/LOW) but apply this
CRM-specific multiplier:

- **Multi-tenant data leak** → always CRITICAL (regardless of exploit difficulty)
- **Webhook replay without idempotency** → always CRITICAL (financial/reputation risk)
- **Missing RBAC on admin endpoint** → always HIGH
- **Debug mode in prod** → always CRITICAL (immediate full compromise)
- **Missing rate limit on auth** → HIGH
- **Verbose error in prod** → MEDIUM

### Output report format (override upstream if needed)

Save reports to `.gstack/cso-reports/cso-report-{date}.md` (NOT the global
`~/.gstack/projects/`). Include the disclaimer from the original skill.

Always include a **CRM-specific remediation priority** section at the end of
the report, listing findings ordered by: tenant-isolation risk > webhook idempotency
